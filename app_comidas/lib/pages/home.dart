...
class _HomeContentState extends State<HomeContent> {
  ...
  List<ListTile> _buildSuggestions(SearchController controller) {
    return _locales
        .where((local) {
          return local['title'].toString().toLowerCase().contains(
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) => LocalScreen(
                    localId: local['loc_id'] ?? local['id'] ?? 1,
                    localName: local['title'] ?? 'Local',
                  ),
                ),
              );
            },
  ...
  Widget _buildLocalCard(Map<String, dynamic> local) => Card(
    ...
    child: Column(
      ...
      children: [_buildLocalImage(local), _buildLocalContent(local)],
    ),
  );
        ...
        MaterialPageRoute(
          builder: (_) => LocalScreen(
            localId:
                local['loc_id'] ??
                local['id'] ??
                1, // Ajusta según tu estructura
            localName: local['title'] ?? local['loc_nombre'] ?? 'Local',
          ),
        ),
      ),
